                   ...
                   DecayType* out_decay_type)
{
    ...
    if (!f) return -1; // no file -> caller decides
...
int main(int argc, char** argv){
	...
	read_cli_state(AGENT_CLI_STATE_FILE, &map_path, &lr, &dr, &eps_decay, &sucess_window_size, &sucess_treshold,&current_decay_type);
	if(map_path){
    ...
    while(1){
        ...
        if(choice == 3){
			if(map_path){
				...
				current_agent = run_training(&ir,map_path, lr, dr, eps_decay,sucess_window_size, sucess_treshold,current_decay_type);
			} else {
			...
			if(map_path){
            	save_cli_state(AGENT_CLI_STATE_FILE, map_path, lr, dr, eps_decay, sucess_window_size, sucess_treshold,current_decay_type);
			} else {
				printf("\n[ERROR] No Maze Given\n");
			}
			...
			prompt_training_params(suggested_decay_exp,suggested_decay_linear,&lr,&dr,&eps_decay,&sucess_window_size,&sucess_treshold,&current_decay_type);
			save_cli_state(AGENT_CLI_STATE_FILE, map_path, lr, dr, eps_decay, sucess_window_size, sucess_treshold,current_decay_type);
		}
